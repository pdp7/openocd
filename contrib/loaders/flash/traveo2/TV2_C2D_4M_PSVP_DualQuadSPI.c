...
#include "sdl/tviic2d4m/src/mw/smif_mem/cy_smif_s25fl.c"
...
cy_stc_smif_config_t smifConfig = {
    ...
    .rxClk = CY_SMIF_FEEDBACK_CLK,  // BHDT CY_SMIF_INV_OUTPUT_CLK -> CY_SMIF_FEEDBACK_CLK
...
__attribute__((used)) int Init(unsigned long adr, unsigned long clk, unsigned long fnc) {
    ...
    Cy_SMIF_S25FL_ReadConfigurationRegister(SMIF_USED, CY_SMIF_SLAVE_SELECT_0, (uint8_t *)&prevConfigReg0,
                                            &smifContext);
    Cy_SMIF_S25FL_ReadConfigurationRegister(SMIF_USED, CY_SMIF_SLAVE_SELECT_1, (uint8_t *)&prevConfigReg1,
                                            &smifContext);
    ...
    Cy_SMIF_S25FL_ReadConfigurationRegister(SMIF_USED, CY_SMIF_SLAVE_SELECT_0, (uint8_t *)&readConfigReg0,
                                            &smifContext);
    Cy_SMIF_S25FL_ReadConfigurationRegister(SMIF_USED, CY_SMIF_SLAVE_SELECT_1, (uint8_t *)&readConfigReg1,
                                            &smifContext);
    ...
    Cy_SMIF_S25FL_ReadStatusRegister1(SMIF_USED, CY_SMIF_SLAVE_SELECT_0, (uint8_t *)&readStatusReg1_1, &smifContext);
...
__attribute__((used)) int EraseSector(unsigned long adr) {
    ...
    Cy_SMIF_S25FL_WriteEnable(SMIF_USED, D_QUAD_SLAVES, &smifContext);
    Cy_SMIF_S25FL_SectorErase(SMIF_USED, D_QUAD_SLAVES, adr >> 1, &smifContext);
...
__attribute__((used)) int ProgramPage(unsigned long adr, unsigned long sz, unsigned char *buf) {
    ...
    if (Cy_SMIF_MMIO_Program_ExMem(SMIF_USED, D_QUAD_SLAVES, CY_SMIF_S25FL_ID_4QPP, adr >> 1, sz, buf, CY_SMIF_BLOCKING,

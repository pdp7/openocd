...
cy_en_smif_status_t Cy_SMIF_InitDeviceHyperBus(volatile stc_SMIF_DEVICE_t *device, const cy_stc_device_hb_config_t *config)
{
    ...
    /*   Set registers   */
    /*********************/
    device->unCTL.stcField.u2DATA_SEL  = CY_SMIF_DATA_SEL0;
    device->unCTL.stcField.u1WR_EN     = 1u;
    device->unCTL.stcField.u1CRYPTO_EN = 0u;

    if(config->mergeTimeoutEnable)
    {
        device->unCTL.stcField.u3MERGE_TIMEOUT = config->mergeTimeout;
        device->unCTL.stcField.u1MERGE_EN      = 1u;
    }

    if(config->totalTimeoutEnable)
    {
        device->unCTL.stcField.u14TOTAL_TIMEOUT = config->totalTimeout;
        device->unCTL.stcField.u1TOTAL_TIMEOUT_EN = 1u;
    }

    device->unADDR.u32Register = config->addr;
    device->unMASK.u32Register = config->size;

    {
        un_SMIF_DEVICE_ADDR_CTL_t tmp_ADDR_CTL = { .u32Register = 0x00000000ul };
        tmp_ADDR_CTL.stcField.u3SIZE3  = CY_SMIF_XIP_ADDRESS_5_BYTE;
        tmp_ADDR_CTL.stcField.u1DIV2   = 0u;
        device->unADDR_CTL.u32Register = tmp_ADDR_CTL.u32Register;
    }

    // Latency code to acctual latency cycle
    uint8_t dummySize = Cy_SMIF_HB_LatencyCodeToCycle(config->lc_hb);
    ...
    /****************************/
    {
        ...
        tmp_RD_CMD_CTL.stcField.u8CODE     = config->xipReadCmd;
        tmp_RD_CMD_CTL.stcField.u8CODEH    = 0x00;
        tmp_RD_CMD_CTL.stcField.u2WIDTH    = CY_SMIF_WIDTH_OCTAL;
        tmp_RD_CMD_CTL.stcField.u1DDR_MODE = CY_SMIF_DDR;
        tmp_RD_CMD_CTL.stcField.u2PRESENT2 = CY_SMIF_PRESENT_1BYTE;
...

    {
        ...
        tmp_RD_ADDR_CTL.stcField.u2WIDTH     = CY_SMIF_WIDTH_OCTAL;
        tmp_RD_ADDR_CTL.stcField.u1DDR_MODE  = CY_SMIF_DDR;
        device->unRD_ADDR_CTL.u32Register    = tmp_RD_ADDR_CTL.u32Register;
    }

    {
        un_SMIF_DEVICE_RD_MODE_CTL_t tmp_RD_MODE_CTL = { .u32Register = 0x00000000UL };
        tmp_RD_MODE_CTL.stcField.u8CODE      = 0x00;
        tmp_RD_MODE_CTL.stcField.u8CODEH     = 0x00;
        tmp_RD_MODE_CTL.stcField.u2WIDTH     = CY_SMIF_WIDTH_SINGLE;
        tmp_RD_MODE_CTL.stcField.u1DDR_MODE  = CY_SMIF_SDR;
        tmp_RD_MODE_CTL.stcField.u2PRESENT2  = CY_SMIF_NOT_PRESENT;
        device->unRD_MODE_CTL.u32Register    = tmp_RD_MODE_CTL.u32Register;
    }

    {
        un_SMIF_DEVICE_RD_DUMMY_CTL_t tmp_RD_DUMMY_CTL = { .u32Register = 0x00000000UL };

        if(dummySize == 0)
        {
            tmp_RD_DUMMY_CTL.stcField.u5SIZE5    = 0u;
            tmp_RD_DUMMY_CTL.stcField.u2PRESENT2 = CY_SMIF_NOT_PRESENT;
        }
        else
        {
            tmp_RD_DUMMY_CTL.stcField.u5SIZE5    = dummySize - 1u;
            tmp_RD_DUMMY_CTL.stcField.u2PRESENT2 = CY_SMIF_PRESENT_2BYTE;
        }
        device->unRD_DUMMY_CTL.u32Register   = tmp_RD_DUMMY_CTL.u32Register;
    }

    {
        un_SMIF_DEVICE_RD_DATA_CTL_t tmp_RD_DATA_CTL = { .u32Register = 0x00000000UL };
        tmp_RD_DATA_CTL.stcField.u2WIDTH    = CY_SMIF_WIDTH_OCTAL;
        tmp_RD_DATA_CTL.stcField.u1DDR_MODE = CY_SMIF_DDR;
        device->unRD_DATA_CTL.u32Register   = tmp_RD_DATA_CTL.u32Register;
    }

    // <- CRC
    device->unRD_BOUND_CTL.u32Register = 0UL; // <- Note
    ...
    /*****************************/
    {
        ...
        tmp_WR_CMD_CTL.stcField.u8CODE     = config->xipWriteCmd;
        tmp_WR_CMD_CTL.stcField.u8CODEH    = 0x00;
        tmp_WR_CMD_CTL.stcField.u2WIDTH    = CY_SMIF_WIDTH_OCTAL;
        tmp_WR_CMD_CTL.stcField.u1DDR_MODE = CY_SMIF_DDR;
        tmp_WR_CMD_CTL.stcField.u2PRESENT2 = CY_SMIF_PRESENT_1BYTE;
...

    {
        ...
        tmp_WR_ADDR_CTL.stcField.u2WIDTH    = CY_SMIF_WIDTH_OCTAL;
        tmp_WR_ADDR_CTL.stcField.u1DDR_MODE = CY_SMIF_DDR;
...

    {
        ...
        tmp_WR_MODE_CTL.stcField.u8CODE     = 0x00;
        tmp_WR_MODE_CTL.stcField.u8CODEH    = 0x00;
        tmp_WR_MODE_CTL.stcField.u2WIDTH    = CY_SMIF_WIDTH_SINGLE;
        tmp_WR_MODE_CTL.stcField.u1DDR_MODE = CY_SMIF_SDR;
        tmp_WR_MODE_CTL.stcField.u2PRESENT2 = CY_SMIF_NOT_PRESENT;
    ...
    {
        un_SMIF_DEVICE_WR_DUMMY_CTL_t tmp_WT_DUMMY_CTL = { .u32Register = 0x00000000UL };

        tmp_WT_DUMMY_CTL.stcField.u1RWDS_EN = 1u;

        if(dummySize == 0)
        {
            tmp_WT_DUMMY_CTL.stcField.u5SIZE5    = 0u;
            tmp_WT_DUMMY_CTL.stcField.u2PRESENT2 = CY_SMIF_NOT_PRESENT;
        }
        else
        {
            tmp_WT_DUMMY_CTL.stcField.u5SIZE5    = dummySize - 1;
            tmp_WT_DUMMY_CTL.stcField.u2PRESENT2 = CY_SMIF_PRESENT_2BYTE;
        }
        device->unWR_DUMMY_CTL.u32Register   = tmp_WT_DUMMY_CTL.u32Register;
    }

    {
        un_SMIF_DEVICE_WR_DATA_CTL_t tmp_WR_DATA_CTL = { .u32Register = 0x00000000UL };
        tmp_WR_DATA_CTL.stcField.u2WIDTH    = CY_SMIF_WIDTH_OCTAL;
        tmp_WR_DATA_CTL.stcField.u1DDR_MODE = CY_SMIF_DDR;
        device->unWR_DATA_CTL.u32Register   = tmp_WR_DATA_CTL.u32Register;
    }

    device->unCTL.stcField.u1ENABLED = 1u;
...
cy_en_smif_status_t CY_SMIF_ReadHYPERRAM_REG(volatile stc_SMIF_DEVICE_t *device, uint32_t reg_addr, uint16_t* reg_value)
{
    ...
    // MYKT uint32_t savedIntrStatus = Cy_SysLib_EnterCriticalSection();
    ...
    *reg_value = *(uint16_t*)(baseAddr + (reg_addr << 1ul));
    ...
    //MYKT Cy_SysLib_ExitCriticalSection(savedIntrStatus);

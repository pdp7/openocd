 ...
 ** none
 *****************************************************************************/
void Cy_PMIC_Switch_IntToPMIC(uint8_t pmicTrim, cy_en_reghc_control_polarity_t ctrPolarity, cy_en_reghc_status_polarity_t statusPolarity)
{
    ...
    Cy_SysReg_SelectRegHCMode(CY_SYSREG_EXT_PMIC);
    ...
    do
    {
        retStatus = Cy_SysReg_RegHCPmicStatus();
    } while(retStatus == false);

    // Enable overcurrent detection and supply monitoring resets/interrupts as desired

    // Mask the internal (htreg) regulator error detect signal
 ...
 ** none
 *****************************************************************************/
void Cy_PMIC_Switch_IntToPassTR(uint8_t pmicTrim)
{
    bool retStatus = false;
    ...
    Cy_SysReg_SelectRegHCMode(CY_SYSREG_EXT_TR);
    ...
    // Wait for the s40ereghc or external PMIC power ok signal
    while((Cy_SysReg_IsRegHCCktEnabledAndOperating() == 0) ||
       (Cy_SysReg_IsRegHCOcdWithinLimits() == 0) )
...
////////////////////////////////////////////////////////////////////////////////
void Cy_PassTR_ManuallyEnable(uint8_t vadj)
{
    // Set external pass transistor mode
    Cy_SysReg_SelectRegHCMode(CY_SYSREG_EXT_TR);

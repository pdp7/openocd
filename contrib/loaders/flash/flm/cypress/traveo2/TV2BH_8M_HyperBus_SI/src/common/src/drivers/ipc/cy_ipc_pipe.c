...
void Cy_IPC_Pipe_Init(cy_stc_ipc_pipe_config_t const *config)
{
    ...
    cy_stc_sysint_irq_t                ipc_intr_cypipeConfig;
    ...
    ipc_intr_cypipeConfig.sysIntSrc     = (cy_en_intr_t)(cpuss_interrupts_ipc_0_IRQn + epConfigDataA.ipcNotifierNumber);
    ipc_intr_cypipeConfig.intIdx        = epConfigDataA.ipcNotifierCpuIntIdx;
    ipc_intr_cypipeConfig.isEnabled     = true;
    ...
    (void)Cy_SysInt_InitIRQ(&ipc_intr_cypipeConfig);
    Cy_SysInt_SetSystemIrqVector(ipc_intr_cypipeConfig.sysIntSrc, Cy_IPC_SystemPipeIsr);
}
...
void Cy_IPC_Pipe_EndpointInit(uint32_t epAddr, cy_ipc_pipe_callback_array_ptr_t cbArray,
                              uint32_t cbCnt, uint32_t epConfig, cy_stc_sysint_irq_t const *epInterrupt)
{
    ...
    if (NULL != epInterrupt)
    {
        endpoint->pipeIntrSrc     = epInterrupt->sysIntSrc;
    }
}

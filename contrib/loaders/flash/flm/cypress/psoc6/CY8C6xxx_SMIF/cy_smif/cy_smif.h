...
* \defgroup group_smif_macros_cache Cache Macros
...
typedef struct
{
	...
	uint32_t deselectDelay;     /**<  Specifies the minimum duration of SPI de-selection between SPI transfers:
								...
								*   - "6": 7 clock cycles.
								*   - "7": 8 clock cycles. */
	uint32_t rxClockSel;        /**< Specifies the clock source for the receiver clock:
								*   - "0" SPI clock.
								*   - "1" SPI clock inverted.
								*   - "2" SPI clock feedback.
								*   - "3" SPI clock feedback inverted. */
	/**
...
__STATIC_INLINE void Cy_SMIF_PushTxFifo(SMIF_Type *baseaddr, cy_stc_smif_context_t *context)
{
	...
	while (0U != writeBytes)
	{
		...
		{
			baseaddr->TX_DATA_FIFO_WR4 = ((uint32_t*)buff)[0U];
			baseaddr->TX_DATA_FIFO_WR4 = ((uint32_t*)buff)[1U];
		}
		...
		{
			baseaddr->TX_DATA_FIFO_WR2 = ((uint16_t*)buff)[0U];
		}
		...
		{
			baseaddr->TX_DATA_FIFO_WR4 = ((uint32_t*)buff)[0U];
			baseaddr->TX_DATA_FIFO_WR2 = ((uint16_t*)buff)[2U];
		}
...
__STATIC_INLINE void Cy_SMIF_PopRxFifo(SMIF_Type *baseaddr, cy_stc_smif_context_t *context)
{
	...
	while (0U != readBytes)
	{
		...
		{
			((uint32_t*)buff)[0U] = baseaddr->RX_DATA_FIFO_RD4;
			((uint32_t*)buff)[1U] = baseaddr->RX_DATA_FIFO_RD4;
		}
		...
		{
			((uint16_t*)buff)[0U] = (uint16_t)baseaddr->RX_DATA_FIFO_RD2;
		}
		...
		{
			((uint32_t*)buff)[0U] = baseaddr->RX_DATA_FIFO_RD4;
			((uint16_t*)buff)[2U] = (uint16_t)baseaddr->RX_DATA_FIFO_RD2;
		}

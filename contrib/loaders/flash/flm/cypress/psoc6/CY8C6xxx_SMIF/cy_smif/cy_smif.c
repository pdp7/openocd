...
*
*******************************************************************************/
cy_en_smif_status_t Cy_SMIF_Init(SMIF_Type *base, cy_stc_smif_config_t const *config, uint32_t timeout,
	...
	if ((NULL != base) && (NULL != config) && (NULL != context)) {
		...
		Cy_SMIF_SetInterruptMask(base,
								 Cy_SMIF_GetInterruptMask(base) & ~(SMIF_INTR_TR_TX_REQ_Msk | SMIF_INTR_TR_RX_REQ_Msk));

		/* Configure the SMIF interface */
		base->CTL = (uint32_t)(
			_VAL2FLD(SMIF_CTL_XIP_MODE, config->mode) | _VAL2FLD(SMIF_CTL_CLOCK_IF_RX_SEL, config->rxClockSel) |
			_VAL2FLD(SMIF_CTL_DESELECT_DELAY, config->deselectDelay) | _VAL2FLD(SMIF_CTL_BLOCK, config->blockEvent));
...
void Cy_SMIF_SetDataSelect(SMIF_Type *base, cy_en_smif_slave_select_t slaveSelect,
	...
	if (NULL != device) {
		device->CTL = _CLR_SET_FLD32U(device->CTL, SMIF_DEVICE_CTL_DATA_SEL, (uint32_t)dataSelect);
...
*
*******************************************************************************/
cy_en_smif_status_t Cy_SMIF_TransmitCommand(SMIF_Type *base, uint8_t cmd, cy_en_smif_txfr_width_t cmdTxfrWidth,
	...
	uint32_t const constCmdPart =
		(_VAL2FLD(CY_SMIF_CMD_FIFO_WR_MODE, CY_SMIF_CMD_FIFO_TX_MODE) | _VAL2FLD(CY_SMIF_CMD_FIFO_WR_SS, slaveSelect));
	uint32_t timeoutUnits = context->timeout;

	/* Send the command byte */
	base->TX_CMD_FIFO_WR = constCmdPart | _VAL2FLD(CY_SMIF_CMD_FIFO_WR_WIDTH, (uint32_t)cmdTxfrWidth) |
						   _VAL2FLD(CY_SMIF_CMD_FIFO_WR_TXDATA, (uint32_t)cmd) |
						   _VAL2FLD(CY_SMIF_CMD_FIFO_WR_LAST_BYTE, ((0UL == paramSize) ? cmpltTxfr : 0UL));
	...
	while ((bufIndex < paramSize) && (CY_SMIF_EXCEED_TIMEOUT != result)) {
		...
		if (Cy_SMIF_GetCmdFifoStatus(base) < CY_SMIF_TX_CMD_FIFO_STATUS_RANGE) {
			base->TX_CMD_FIFO_WR =
				constCmdPart | _VAL2FLD(CY_SMIF_CMD_FIFO_WR_TXDATA, (uint32_t)cmdParam[bufIndex]) |
				_VAL2FLD(CY_SMIF_CMD_FIFO_WR_WIDTH, (uint32_t)paramTxfrWidth) |
				_VAL2FLD(CY_SMIF_CMD_FIFO_WR_LAST_BYTE, ((((uint32_t)bufIndex + 1UL) < paramSize) ? 0UL : cmpltTxfr));
...
*
*******************************************************************************/
cy_en_smif_status_t Cy_SMIF_TransmitData(SMIF_Type *base, uint8_t *txBuffer, uint32_t size,
										 cy_en_smif_txfr_width_t transferWidth, cy_smif_event_cb_t TxCmpltCb,
										 cy_stc_smif_context_t *context) {
	/* The return variable */
	cy_en_smif_status_t result = CY_SMIF_BAD_PARAM;
	...
	if (size > 0U) {
		...
		if (Cy_SMIF_GetCmdFifoStatus(base) < CY_SMIF_TX_CMD_FIFO_STATUS_RANGE) {
			...
			base->TX_CMD_FIFO_WR = _VAL2FLD(CY_SMIF_CMD_FIFO_WR_MODE, CY_SMIF_CMD_FIFO_TX_COUNT_MODE) |
...
*
*******************************************************************************/
cy_en_smif_status_t Cy_SMIF_TransmitDataBlocking(SMIF_Type *base, uint8_t *txBuffer, uint32_t size,
...
*
*******************************************************************************/
cy_en_smif_status_t Cy_SMIF_ReceiveData(SMIF_Type *base, uint8_t *rxBuffer, uint32_t size,
										cy_en_smif_txfr_width_t transferWidth, cy_smif_event_cb_t RxCmpltCb,
										cy_stc_smif_context_t *context) {
	/* The return variable */
	cy_en_smif_status_t result = CY_SMIF_BAD_PARAM;
	...
	if (size > 0U) {
		...
		if (Cy_SMIF_GetCmdFifoStatus(base) < CY_SMIF_TX_CMD_FIFO_STATUS_RANGE) {
			...
			base->TX_CMD_FIFO_WR = _VAL2FLD(CY_SMIF_CMD_FIFO_WR_MODE, CY_SMIF_CMD_FIFO_RX_COUNT_MODE) |
								   _VAL2FLD(CY_SMIF_CMD_FIFO_WR_WIDTH, (uint32_t)transferWidth) |
			...
			if (NULL != rxBuffer) {
				...
				Cy_SMIF_SetInterruptMask(base, Cy_SMIF_GetInterruptMask(base) | SMIF_INTR_TR_RX_REQ_Msk);
			}
			result = CY_SMIF_SUCCESS;
		}
	}
...
*
*******************************************************************************/
cy_en_smif_status_t Cy_SMIF_ReceiveDataBlocking(SMIF_Type *base, uint8_t *rxBuffer, uint32_t size,
												cy_en_smif_txfr_width_t transferWidth,
												cy_stc_smif_context_t const *context) {
	/* The return variable */
	cy_en_smif_status_t result = CY_SMIF_BAD_PARAM;
...
cy_en_smif_status_t Cy_SMIF_SendDummyCycles(SMIF_Type *base, uint32_t cycles) {
	...
	if (Cy_SMIF_GetCmdFifoStatus(base) < CY_SMIF_TX_CMD_FIFO_STATUS_RANGE) {
		...
		base->TX_CMD_FIFO_WR = _VAL2FLD(CY_SMIF_CMD_FIFO_WR_MODE, CY_SMIF_CMD_FIFO_DUMMY_COUNT_MODE) |
...
*
*******************************************************************************/
cy_en_smif_status_t Cy_SMIF_Encrypt(SMIF_Type *base, uint32_t address, uint8_t data[], uint32_t size,
									cy_stc_smif_context_t const *context) {
	uint32_t bufIndex;
	...
	if ((NULL != data) && ((address & (~CY_SMIF_CRYPTO_ADDR_MASK)) == 0UL)) {
		...
		for (bufIndex = 0U; bufIndex < (size / CY_SMIF_AES128_BYTES); bufIndex++) {
			...
			base->CRYPTO_INPUT0 = (uint32_t)(address + ((bufIndex * CY_SMIF_AES128_BYTES) & CY_SMIF_CRYPTO_ADDR_MASK));

			/* Start the encryption */
			base->CRYPTO_CMD &= ~SMIF_CRYPTO_CMD_START_Msk;
			base->CRYPTO_CMD = (uint32_t)(_VAL2FLD(SMIF_CRYPTO_CMD_START, CY_SMIF_CRYPTO_START));

			while ((CY_SMIF_CRYPTO_COMPLETED != _FLD2VAL(SMIF_CRYPTO_CMD_START, base->CRYPTO_CMD)) &&
			...
			Cy_SMIF_UnPackByteArray(base->CRYPTO_OUTPUT0, &cryptoOut[CY_SMIF_CRYPTO_FIRST_WORD], true);
			Cy_SMIF_UnPackByteArray(base->CRYPTO_OUTPUT1, &cryptoOut[CY_SMIF_CRYPTO_SECOND_WORD], true);
			Cy_SMIF_UnPackByteArray(base->CRYPTO_OUTPUT2, &cryptoOut[CY_SMIF_CRYPTO_THIRD_WORD], true);
			Cy_SMIF_UnPackByteArray(base->CRYPTO_OUTPUT3, &cryptoOut[CY_SMIF_CRYPTO_FOURTH_WORD], true);
...
*
* \param cacheType
* Holds the type of the cache to be modified. The supported values:
*   -   \ref CY_SMIF_CACHE_SLOW
*   -   \ref CY_SMIF_CACHE_FAST
*   -   \ref CY_SMIF_CACHE_BOTH
*
*******************************************************************************/
void Cy_SMIF_CacheEnable(SMIF_Type *base, uint32_t cacheType) {
	switch (cacheType) {
...
*
*******************************************************************************/
void Cy_SMIF_CacheDisable(SMIF_Type *base, uint32_t cacheType) {
	switch (cacheType) {
		case CY_SMIF_CACHE_SLOW:
			base->SLOW_CA_CTL &= ~SMIF_SLOW_CA_CTL_ENABLED_Msk;
			break;
...
*
*******************************************************************************/
void Cy_SMIF_CachePrefetchingEnable(SMIF_Type *base, uint32_t cacheType) {
	switch (cacheType) {
		case CY_SMIF_CACHE_SLOW:
...
*
*******************************************************************************/
void Cy_SMIF_CachePrefetchingDisable(SMIF_Type *base, uint32_t cacheType) {
	switch (cacheType) {
		case CY_SMIF_CACHE_SLOW:
			base->SLOW_CA_CTL &= ~SMIF_SLOW_CA_CTL_PREF_EN_Msk;
			break;
...
*
*******************************************************************************/
void Cy_SMIF_CacheInvalidate(SMIF_Type *base, uint32_t cacheType) {
	switch (cacheType) {
		case CY_SMIF_CACHE_SLOW:
...
cy_en_syspm_status_t Cy_SMIF_DeepSleepCallback(cy_stc_syspm_callback_params_t *callbackParams) {
	...
	switch (callbackParams->mode) {
		case CY_SYSPM_CHECK_READY: {
			 ...
			 */
			bool checkFail = (CY_SMIF_REC_BUSY == (cy_en_smif_txfr_status_t)Cy_SMIF_GetTxfrStatus(locBase, locContext));
